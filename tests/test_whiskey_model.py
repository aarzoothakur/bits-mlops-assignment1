...
def preprocess_data():
    ...
    # Apply Label Encoding to 'whiskey_quality' to convert categorical labels to numerical labels
...
def test_accuracy(load_model, preprocess_data):
    ...
    assert accuracy >= ACCURACY_THRESHOLD, f"Accuracy is below expected threshold: {accuracy:.2f}"

# Test the classification report for specific performance metrics
...
def test_confusion_matrix(load_model, preprocess_data):
    ...
    # Assert that there are no zero values in the diagonal (meaning no class was missed completely)
    diagonal_values = np.diag(conf_matrix)
    assert all(diagonal_values > 0), f"Confusion matrix has zero values on the diagonal: {diagonal_values}"
